...
@nodes.promote(
    tag="promote", pipeline="simple", depends_on=["group_by"], env=["prod", "backfill"]
)
def promote_prod(group_by_df: DataFrame) -> None:
    """Promotes validated data to production promoted table."""
    group_by_df.write.option(
        "path", "/tmp/requete/spark/warehouse/table_promoted"
    ).mode("overwrite").saveAsTable("table_promoted")
...
def test_promote_staging(group_by_df: DataFrame):
    ...
    assert group_by_df.count() > 0, "Data should not be empty"
    assert group_by_df.filter(group_by_df.c_modified.isNull()).count() == 0, (
        "Nulls found in c_modified"
    )
